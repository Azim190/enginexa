...
// GET /api/users - Read employee profiles with branch/dept/reporting line
router.get('/', authenticateSession, requirePermissions(['users:read']), async (req, res) => {
    try {
        const db = await openDb();
        // Query to select users and details
        const users = await db.all(`
            SELECT u.id, u.name, u.id_number, u.role, u.branch_id, u.department_id, u.reporting_line_id, u.mfa_enabled, u.status, u.mfa_pin, u.createdAt,
...
router.post('/', authenticateSession, requirePermissions(['users:write']), async (req, res) => {
    try {
        ...
        const duplicate = await db.get('SELECT id FROM users WHERE id_number = ?', [id_number.trim()]);
        if (duplicate) {
        ...
        const result = await db.run(
            `INSERT INTO users (name, id_number, password_hash, role, branch_id, department_id, reporting_line_id, mfa_enabled, mfa_pin, status, createdAt)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [
                name.trim(),
                id_number.trim(),
...
router.put('/:id', authenticateSession, requirePermissions(['users:write']), async (req, res) => {
    try {
        ...
        if (id_number) {
            const duplicate = await db.get('SELECT id FROM users WHERE id_number = ? AND id != ?', [id_number.trim(), id]);
            if (duplicate) {
                return res.status(409).json({ error: 'Another user with this ID number already exists' });
            }
        }
...
router.delete('/:id', authenticateSession, requirePermissions(['users:write']), async (req, res) => {
    try {
        ...
        const reports = await db.get('SELECT id FROM users WHERE reporting_line_id = ?', [id]);
        if (reports) {
