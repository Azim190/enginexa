...
export const Sidebar = ({ isOpen, onClose }: SidebarProps) => {
    ...
    return (
        <>
            <aside
                className={clsx(
                    'fixed inset-y-0 start-0 z-50 w-64 flex flex-col transition-transform duration-300 ease-in-out',
                    'md:sticky md:top-0 md:h-screen md:!translate-x-0',
                    'bg-brand-600 border-r border-brand-700 rtl:border-r-0 rtl:border-l rtl:border-brand-700',
                    isOpen ? 'translate-x-0' : '-translate-x-full rtl:translate-x-full'
                )}
            >
                {/* Mashrabiya texture */}
                <div className="mashrabiya-overlay absolute inset-0 pointer-events-none" />
                ...
                {/* ── Logo header ── */}
                <div className="relative z-10 px-5 py-6 border-b border-white/10 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-white rounded-xl flex items-center justify-center flex-shrink-0 shadow-lg p-1.5">
                        ...
                        <div>
                            ...
                            <p className="text-brand-300 text-[10px] tracking-wider mt-0.5">
                    ...
                    <button
                        onClick={onClose}
                        className="md:hidden p-2 text-brand-300 hover:text-white hover:bg-white/10 rounded-xl transition-colors"
                    >
                        <X className="w-4 h-4" />
                    </button>
                </div>
                ...
                {/* ── Gold accent line ── */}
                <div className="relative z-10 h-px bg-gradient-to-r from-transparent via-gold-400/60 to-transparent" />

                {/* ── Navigation ── */}
                <nav className="relative z-10 flex-1 p-4 space-y-1 overflow-y-auto scrollbar-hide">
                    <p className="text-brand-400 text-[10px] uppercase tracking-widest font-semibold px-4 mb-3">
                        ...
                        <div className="mt-2 pt-2 border-t border-white/5">
                            <button
                                ...
                                className={clsx(
                                    ...
                                    isDesignOpen && 'bg-white/10 text-white'
                                )}
                            >
                                <div className="flex items-center gap-3">
                                    <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-white/5 flex-shrink-0">
                                ...
                                <ChevronRight
                                    className={clsx(
                                        'w-3.5 h-3.5 transition-transform duration-200 text-brand-300',
                            ...
                            {isDesignOpen && (
                                <div className="ps-11 mt-1 space-y-1 border-l border-white/10 ms-4 rtl:border-l-0 rtl:border-r rtl:border-white/10 rtl:pe-11 rtl:ps-0 rtl:me-4">
                                    {filteredDesignItems.map(item => (
                                        <NavLink
                                            ...
                                            onClick={() => { if (window.innerWidth < 768) onClose(); }}
                                            className={({ isActive }) => clsx(
                                                'nav-link py-2 text-xs opacity-80 hover:opacity-100',
                                                isActive && 'nav-link-active !opacity-100 font-semibold text-white bg-white/5'
                                            )}
                                        >
                    ...
                    {user && canAccessSupervision && (
                        <div className="mt-2 pt-2 border-t border-white/5">
                            <button
                                ...
                                className={clsx(
                                    'nav-link w-full text-start flex items-center justify-between',
                                    isSupervisionOpen && 'bg-white/10 text-white'
                                )}
                            >
                                <div className="flex items-center gap-3">
                                    <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-white/5 flex-shrink-0">
                ...
                {user && ['admin', 'ceo', 'branch manager', 'secretary'].includes(user.role.toLowerCase()) && (
                    <div className="mt-2 pt-2 border-t border-white/5">
                        <button
                            ...
                            className={clsx(
                                'nav-link w-full text-start flex items-center justify-between',
                                isCorrOpen && 'bg-white/10 text-white'
                            )}
                        >
                            <div className="flex items-center gap-3">
                                <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-white/5 flex-shrink-0">
                ...
                {/* ── User info + logout ── */}
                <div className="relative z-10 border-t border-white/10 p-4 space-y-3">
                    {/* User card */}
                    <div className="flex items-center gap-3 px-3 py-2.5 bg-white/5 rounded-xl border border-white/10">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <p className="text-brand-300 text-[10px] capitalize truncate">
                    ...
                    <button
                        onClick={logout}
                        className="flex items-center gap-3 w-full px-4 py-2.5 text-brand-300 hover:bg-red-500/20 hover:text-red-300 rounded-xl transition-all duration-200 text-sm font-medium"
                    >
